...
func TestBoxRecording(t *testing.T) {
	...
	// when
	box := startBoxWithEnv(ctx, t, boxName, network.name, map[string]string{})
	defer box.Terminate(ctx)

	testerName := randomString("record-tester-")
	err := startRecordTester(ctx, t, testerName, network.name)
	require.NoError(t, err)
}

func startBoxWithEnv(ctx context.Context, t *testing.T, hostname, network string, env map[string]string) *catalystContainer {
...
func startRecordTester(ctx context.Context, t *testing.T, hostname, network string) error {
	// Mostly here to specify the dependency
	glog.Infof("booting up record-tester version %s", model.Version)
	err := run(
		ctx,
		"go",
		"run",
